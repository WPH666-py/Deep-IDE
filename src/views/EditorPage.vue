    ...
    <div class="editor-header">
      ...
      <div style="position:relative">
        ...
        <div class="dropdown-menu" :class="{ show: openDropdown === 'startMenu' }">
          ...
          <div class="dropdown-divider"></div>
          <div class="dropdown-item" style="color:#e74c3c" @click="uninstallApp">卸载 Deep IDE</div>
      ...
      <div style="display:flex;align-items:center;gap:0.2rem">
        ...
        <div style="position:relative">
          <button
            ...
            :disabled="store.toolCalls.length === 0 && !(store.isLoading && store.useTools)"
          >
          ...
          <div class="ai-input-area">
            ...
            <div class="ai-send-row">
              <select v-model="store.currentMode" @change="store.switchMode(store.currentMode)">
                ...
                <option value="deep-anth">DeepAnth</option>
                <option value="deep-oai">DeepOAI</option>
                <option value="deep-gem">DeepGem</option>
                <option value="deep-qwen">DeepQwen</option>
                <option value="deep-kimi">DeepKimi</option>
              </select>
              <label class="ai-tools-toggle" title="启用 Claude Code / Cursor 风格的 9 工具 Agent Loop">
                <input type="checkbox" v-model="store.useTools" />
                <span>🛠 工具</span>
              </label>
        ...
        <div class="modal-body">
          ...
          <div style="margin-top:2rem;padding-top:1.2rem;border-top:1px solid #eee">
            ...
            <div style="font-size:0.85rem;color:#555;line-height:1.7">
              ...
              <div>👤 <b>水哥</b></div>
              <div>💡 Deep IDE，新一代智能体IDE。用最简洁的架构，做最牛逼的产品！</div>
...
async function gitPush() {
  ...
  try {
    const result = await invoke<string>("git_push", {
      ...
      message: gitCommitMsg.value || "update from Deep IDE"
    });
...
.config-field input { width: 100%; padding: 0.4rem 0.5rem; border: 1px solid #e0e0e0; border-radius: 4px; font-size: 0.82rem; outline: none; }
